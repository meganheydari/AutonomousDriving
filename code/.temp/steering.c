...
void init(void)
{
    ...
    mcpwm_set_duty_in_us(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_B, 1600); // NEUTRAL signal in microseconds
    ...
    while (1) {
        ...
        for(i = 0; i < len - 1; i++) {
            ...
            if (data[i] == 0x59 && prev == 0x59) {
                distanceBack = data[i + 1] + (data[i+2]*256);
                //printf("data: %d\n", data[i + 1]);
            }
...
void microLidar2()
{
    ...
    while (1) {
        ...
        for(i = 0; i < len - 1; i++) {
            ...
            if (data[i] == 0x59 && prev == 0x59) {
                distanceFront = data[i + 1] + (data[i+2]*256);
                //printf("data: %d\n", data[i + 1]);
            }
...
void app_main(void)
{
    ...
    while(1) {
        ...
        integral = integral + (distanceFront - prev_distance) * 10;
        ...
        prev_distance = distanceFront;
        localAngle = integral + (distanceFront - distanceBack) + 45;
